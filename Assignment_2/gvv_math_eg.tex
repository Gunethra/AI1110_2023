...
\textbf{Solution:}
Let X be a random variable defined as the value given by the pointer. Probability of the pointer pointing a number is;
\begin{align}
    p = \frac{1}{8}\\
    n = 8
\end{align}
...
\begin{align}
    F_{X} \brak{i} &= \sum_{0}^{i} {\comb{n}{i} \times p^{i} \times (1-p)^ \brak {n-i}}
\end{align}
...
\item 
For i = \cbrak{3, 4, 5, 6, 7, 8}, required probability is equivalent to;
\begin{align}
...
\item 
For i = \cbrak{1, 2, 3, 4, 5, 6, 7, 8}, required probability is equivalent to;
\begin{align}
